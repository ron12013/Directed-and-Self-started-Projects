...
def main(_):
	...
	mnist = input_data.read_data_sets("MNIST_data/", one_hot = True)
        ###Enter code to define the necessary placeholders we need for this program###
	#SOLUTION:
	...
	###Enter code to define the necessary tensorflow variables, note that MNIST images are 28 * 28###
	...
	###Enter the code for the usage of weights and the matrix multiplication. ###
	
	# SOLUTION:
	#The matrix multiplication takes place here where we get the logits and use it later in softmax layer
	y = tf.matmul(x,W) + b

	### Enter the code to calculate the cross entropy loss.
	# Use the tf.nn.softmax_cross_entropy_with_logits for that, then use tf.reduce_mean.
	...
	### Enter Code: Minimize the loss with the tensorflow's gradient descent optimizer.
